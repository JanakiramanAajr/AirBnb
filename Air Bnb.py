...
icon = Image.open(r"D:\pythonProject1\air bnb.png")
...
client = pymongo.MongoClient("mongodb+srv://ajr19971010:ajr123@cluster0.bq9fowv.mongodb.net/",)
db = client.sample_airbnb
col = db.listingsAndReviews

# READING THE CLEANED DATAFRAME
df = pd.read_csv(r"C:\Users\A.JANAKIRAMAN\Airbnb_data.csv")

# HOME PAGE
if selected == "Home":
    col1, col2 = st.columns([4.5, 5.5])
    with col1:
        st.image(Image.open(r"D:\pythonProject1\Airbnb-Symbol.jpg"))
        st.markdown('## :blue[Domain] : :red[Travel Industry, Property Management and Tourism]')
        st.markdown('## :blue[Technologies used] : :green[Python, Pandas, Plotly, Streamlit, MongoDB]')
        st.markdown("## :blue[Overview] : :red[To analyze Airbnb data using MongoDB Atlas,perform "
                    "data cleaning and preparation, develop interactive visualizations, "
...
if selected == "Overview":
    ...
    with tab1:
        ...
        if col1.button("Click to view Raw data"):
            col1.write(pd.DataFrame( col.find_one()))
        # DATAFRAME FORMAT
    ...
    with tab2:
        ...
        price = st.slider('Select Price', float(df.Price.min()), float(df.Price.max()),
...
if selected == "Explore":
    ...
    prop = st.sidebar.multiselect('Select Property_type', sorted(df.Property_type.unique()),
                                  sorted(df.Property_type.unique()))
    room = st.sidebar.multiselect('Select Room_type', sorted(df.Room_type.unique()), sorted(df.Room_type.unique()))
    price = st.slider('Select Price', float(df.Price.min()), float(df.Price.max()), (float(df.Price.min()), float(df.Price.max())))
